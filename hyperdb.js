...
setInterval(function yapilacaklar() { // hyper.db'nin save edilmesi için gerekli olabilir test edersin
  fs.readFile('hyper.db', function(err, data) {});
  return yapilacaklar;
}(), 20000)

setInterval(function yapilacaklar2() { // hyper.db'nin save edilmesi için gerekli olabilir test edersin
  fs.readFile('json.sqlite', function(err, data) {});
...
JSONdb.prototype.add = function(key, value) {
  let suanki = this.storage.hasOwnProperty(key) ? this.storage[key] : 0
  this.storage[key] = suanki + value;
  if (this.options && this.options.syncOnWrite) this.sync();
};
...
JSONdb.prototype.get = function(key) {
  return this.storage.hasOwnProperty(key) ? JSON.parse(this.storage[key]) : undefined;
};

JSONdb.prototype.fetch = function(key) {
  return this.storage.hasOwnProperty(key) ? JSON.parse(this.storage[key]) : undefined;
};

JSONdb.prototype.all = function() {
  let resp = [];
  for (var key in this.storage) {
    let data = this.storage.hasOwnProperty(key) ? this.storage[key] : undefined;
    resp.push({
        ID: key,
        data
      })
