...
//yukardakiler tam nasıl çaılışıyor en iyi çalışan sistemi bulup değişirsin şuanki iyiyse kalabilir
...
module.exports = {
  set: async function (key, newvalue) {
    ...
    {
      fs.appendFile('./hyper.db', key+"->"+newvalue.toString()+"\n", function (err) {if (err) return console.log(err);});
      return
    }
  ...
  add: async function (key, addition) {
    ...
    if(!data.includes(key))
    {
      fs.appendFile('./hyper.db', key+"->"+addition.toString()+"\n", function (err) {if (err) return console.log(err);});
      return
    }
