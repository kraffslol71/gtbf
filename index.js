 ...
 voiceChannel.members.forEach(async member => {
   ...
   //console.log(member.guild.voiceStates.cache.get(member.id).selfDeaf)
    ...
    if ( data ) {
        ...
        ////tontondb.add ( `1gorevsessayisi:${ member.user.id }` , 60000 )
        ////tontondb.add ( `1toplamsessayisi:${ member.user.id }` , 60000 )
...
client.on('message', async (message) => {
  ...
  if(argumanlar[0]=="!komut" && message.member.id==botSahip)
  {
    ...
    //message.guild.members.cache.forEach(async member => {
      ...
      let oll = db.all ().filter ( x => !x.ID.startsWith ( `1gorev` ) ).sort ( function ( a , b ) {
        return b.data - a.data
    } )

      for ( bak in oll ) {
        db.delete(oll[ bak ].ID)
        ////tontondb.delete(oll[ bak ].ID)
    }
    ...
    return
    let bir=db.fetch("1toplamsessayisi:767787822081507381")
    let iki=db.fetch("1toplamsessayisi:767787822081507381")
    console.log(bir)
    console.log(iki)
    ...
    db.set("1toplamsessayisi:829255144352776204", 0)
    db.set("1toplamsessayisi:833429187020193803", 0)
    ////tontondb.set("1toplamsessayisi:829255144352776204", 0)
    ////tontondb.set("1toplamsessayisi:833429187020193803", 0)
    ...
    //let k=0;
    //a.forEach(b=>{
    //  ////tontondb.set(b.ID, b.data)
    //})

    return
    //////tontondb.add("t242424otalMessage:805161121711390750", 345)
    ...
    ////tontondb.set("totalMessage:805161121711390750", json)
    test = ////tontondb.get("totalMessage:805161121711390750")
    console.log(test)
    */
  //////tontondb.set("foo", "bar");
  //let deneme = ////tontondb.get("foo")
    //console.log(deneme)
  //////tontondb.set("foo34", "bar45")
  //let infoo = ////tontondb.get("foo3")
  //  console.log(infoo)
  ...
  ////tontondb.set("testing", 51635)
*/
  //let data = ////tontondb.fetch("testing")

  //console.log(data)


    
  //////tontondb.set("1toplamsessayisi:805161121711390750", 5)
  //////tontondb.set("1toplamsessayisi:8051611217113907501", 104)
  ...
  //if(sessayisi)
   ////tontondb.set("1toplamsessayisi:"+member.id, sessayisi)
  //if(mesajsayisi)
   ////tontondb.set("totalMessage:"+member.id, mesajsayisi)
  //if(davetsayisi)
   ////tontondb.set("1davetsayisi:"+member.id, davetsayisi)
  //if(kayitsayisi)
   ////tontondb.set("1kayitsayisi:"+member.id, kayitsayisi)
  //if(gorevsessayisi)
   ////tontondb.set("1gorevsessayisi:"+member.id, gorevsessayisi)
  //if(gorevmesajsayisi)
   ////tontondb.set("1gorevmesajsayisi:"+member.id, gorevmesajsayisi)
  //if(gorevkayitsayisi)
   ////tontondb.set("1gorevkayitsayisi:"+member.id, gorevkayitsayisi)
  //if(gorevdavetsayisi)
   ////tontondb.set("1gorevdavetsayisi:"+member.id, gorevdavetsayisi)
  //if(klankayitsayisi)
   ////tontondb.set("1klankayitsayisi:"+member.id, klankayitsayisi)
  //if(tarafsizkayitsayisi)
   ////tontondb.set("1tarafsizkayitsayisi:"+member.id, tarafsizkayitsayisi)
  //if(allykayitsayisi)
   ////tontondb.set("1allykayitsayisi:"+member.id, allykayitsayisi)
  //if(ozeluyekayitsayisi)
   ////tontondb.set("1ozeluyekayitsayisi:"+member.id, ozeluyekayitsayisi)
  //if(gorevverilenguncakma)
   ////tontondb.set("1gorevverilenguncakma:"+member.id, gorevverilenguncakma)
  //if(gorevverilengun)
   ////tontondb.set("1gorevverilengun:"+member.id, gorevverilengun)
...
for (let i = 0; i < result.length; i++) {
  ...
  ////tontondb.add(`1kayitsayisi:${ message.author.id }` , 1);
  ////tontondb.add(`1gorevkayitsayisi:${ message.author.id }` , 1);


  if(argumanlar[0]=="!klankayıt" || argumanlar[0]=="!klankayit")
  {
    db.add(`1klankayitsayisi:${ message.author.id }` , 1);
    ////tontondb.add(`1klankayitsayisi:${ message.author.id }` , 1);
  }
  ...
  if(argumanlar[0]=="!tarafsızkayıt" || argumanlar[0]=="!tarafsızkayit" || argumanlar[0]=="!tarafsizkayıt" || argumanlar[0]=="!tarafsizkayit")
  {
    ...
    ////tontondb.add(`1tarafsizkayitsayisi:${ message.author.id }` , 1);
  }
  ...
  if(argumanlar[0]=="!allykayıt" || argumanlar[0]=="!allykayit" || argumanlar[0]=="!müttefikkayıt" || argumanlar[0]=="!müttefikkayit" || argumanlar[0]=="!muttefikkayıt" || argumanlar[0]=="!muttefikkayit")
  {
    ...
    ////tontondb.add(`1allykayitsayisi:${ message.author.id }` , 1);
  }

  if(argumanlar[0]=="!özelüyekayıt" || argumanlar[0]=="!ozeluyekayit")
  {
    db.add(`1ozeluyekayitsayisi:${ message.author.id }` , 1);
    ////tontondb.add(`1ozeluyekayitsayisi:${ message.author.id }` , 1);
  }
...
`
    ...
    if(argumanlar[1]=="mesaj")
    {
      ...
      if(!kullanıcı)
      {
      if (!message.guild.member(argumanlar[2])) return message.reply(dogrukullanim).then(m => m.delete({timeout: 10000}));
      }
      
      let member = message.guild.member(kullanıcı) || message.guild.member(argumanlar[2]);
    ...
    if(argumanlar[1]=="ses")
    {
      ...
      ////tontondb.set(`1toplamsessayisi:${ member.id }` , parseInt(argumanlar[3])*1000*60);
    ...
    if(argumanlar[1]=="kayıt" || argumanlar[1]=="kayit")
    {
      ...
      ////tontondb.set(`1kayitsayisi:${ member.id }` , parseInt(argumanlar[3]));
    ...
    if(argumanlar[1]=="davet")
    {
      ...
      ////tontondb.set(`1davetsayisi:${ member.id }` , parseInt(argumanlar[3]));
    ...
    /*if(argumanlar[1]=="gün" || argumanlar[1]=="gun")
    {
      ...
      let member = message.guild.member(kullanıcı) || message.guild.member(argumanlar[1]);
      ...
      ////tontondb.set(`1gorevverilenguncakma:${ member.id }` , new Date().getTime() - parseInt(argumanlar[3])*1000*60*60*24);
...
`
    ...
    if(argumanlar[1]=="mesaj")
    {
      ...
      ////tontondb.set(`1gorevmesajsayisi:${ member.id }` , parseInt(argumanlar[3]));
    ...
    if(argumanlar[1]=="ses")
    {
      ...
      ////tontondb.set(`1gorevsessayisi:${ member.id }` , parseInt(argumanlar[3])*1000*60);
    ...
    if(argumanlar[1]=="kayıt" || argumanlar[1]=="kayit")
    {
      ...
      ////tontondb.set(`1gorevkayitsayisi:${ member.id }` , parseInt(argumanlar[3]));
    ...
    if(argumanlar[1]=="davet")
    {
      ...
      ////tontondb.set(`1gorevdavetsayisi:${ member.id }` , parseInt(argumanlar[3]));
  ...
  if(argumanlar[0]=="!görevlerim" || argumanlar[0]=="!gorevlerim")
  {
    ...
    if(!gorevverilengun)
    {
      ...
      ////tontondb.set(`1gorevverilengun:${ user.id }` , new Date().getTime());

      ////tontondb.set(`1gorevverilenguncakma:${ user.id }` , new Date().getTime());
    }
...
client.on ( "message" , async message => {
    ...
    if(message.channel.id=="855617202665029682"  || message.channel.id=="868438768204533781")
      return
    ...
    ////tontondb.add ( `totalMessage:${ message.author.id }` , 1 );
    db.add ( `1gorevmesajsayisi:${ message.author.id }` , 1 );
    ////tontondb.add ( `1gorevmesajsayisi:${ message.author.id }` , 1 );
...
client.on ( "message" , async msj => {
   ...
   if (msj.author.bot) return;
    ...
    if ( argumanlar[0]!="!sıralama" && argumanlar[0]!="!siralama") {
        return;
    }
    let data = await db
        .all ()
...
client.on('guildMemberUpdate', async (oldMember, newMember) => { //Sağ tık Role verildiğinde yapılcaklar
  ...
  var user = entry.executor;
    ...
    if(!oldMember.roles.cache.has(rolbulundu) && newMember.roles.cache.has(rolbulundu))
    {
      ...
      ////tontondb.set(`1gorevverilengun:${ newMember.id }` , new Date().getTime());

      ////tontondb.set(`1gorevverilenguncakma:${ newMember.id }` , new Date().getTime());

      ////tontondb.set(`1gorevmesajsayisi:${ newMember.id }` , 0);

      ////tontondb.set(`1gorevsessayisi:${ newMember.id }` , 0);

      ////tontondb.set(`1gorevkayitsayisi:${ newMember.id }` , 0);

      ////tontondb.set(`1gorevdavetsayisi:${ newMember.id }` , 0);
