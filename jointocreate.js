const JOINTOCREATECHANNEL = "935666237709643787"
const jointocreatemap = new Map();
const seslioda = new Map();

module.exports = { seslioda };
...
try {
  ...
  if (oldState.channelID && newState.channelID) {
    ...
    async function jointocreatechannel(user) {
      ...
      await user.guild.channels.create(`${user.member.displayName/*user.username, user.tag, .displayName*/} Odası`, {
        type: 'voice',
        parent: "935666037687476285", //or set it as a category id
      }).then(async vc => {
         
        seslioda.set(`user.id`, vc.id);
        ...
        await vc.overwritePermissions([
          ...
          {
            id: user.guild.me.id, // oda sorumlusu rolü id'si
            allow: 0x11100410,
          },
          {
            id: "933787534700445748", // muteli rolü id'si
            deny: 0x02200200,
          },           
        ]);
      })
